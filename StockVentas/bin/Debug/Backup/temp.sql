...
MySQL - 5.5.0-m2-community : Database - pipogorosito@gmail.com
*********************************************************************
*/
...
CREATE DATABASE /*!32312 IF NOT EXISTS*/`pipogorosito@gmail.com` /*!40100 DEFAULT CHARACTER SET utf8 */;

USE `pipogorosito@gmail.com`;
...
BEGIN
  ...
  INSERT INTO ventash(`Fecha`,`NombreLocal`,`FormaPago`,`TotalPublico`,`TotalCosto`,`Prendas`)
			...
			FROM
				`pipogorosito@gmail.com`.`ventasdiariascons`
				LEFT JOIN `pipogorosito@gmail.com`.`ventash` 
...
IF p_forma = "TODAS" THEN
	...
	FROM
		`pipogorosito@gmail.com`.`pc`
		INNER JOIN `pipogorosito@gmail.com`.`locales` 
			ON (`pc`.`IdLocalPC` = `locales`.`IdLocalLOC`)
		INNER JOIN `pipogorosito@gmail.com`.`ventas` 
			ON (`ventas`.`IdPCVEN` = `pc`.`IdPC`)
		INNER JOIN `pipogorosito@gmail.com`.`ventasdetalle` 
			ON (`ventasdetalle`.`IdVentaDVEN` = `ventas`.`IdVentaVEN`)
		INNER JOIN `pipogorosito@gmail.com`.`formaspago` 
